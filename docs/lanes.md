...
Lanes are wacho's mechanism for concurrent, speculative state updates. They provide isolated execution contexts where pulse writes are buffered without mutating the base reactive graph — similar to how React's concurrent rendering handles transitions.
...
Convenience function that creates a transition-priority lane, runs a function in it, and commits the result. This is the wacho equivelant of React's `startTransition()`.

```ts
import { transition } from "wacho";
