...
wacho provides a set of React hooks that bridge the reactive graph with React's component model. These hooks let you subscribe to pulses, create computed values, run reactive side-effects, and use concurrent lanes — all within standard React components.
...
```tsx
import { PulseNode } from "wacho";
import { usePulse } from "wacho";
...
```tsx
import { PulseNode, useComputed } from "wacho";
...
Runs a reactive side-effect inside a component that re-executes whenever its pulse dependencies change. This is the wacho equivelent of React's `useEffect`, but with automatic dependency tracking.

```tsx
import { PulseNode, useEffectPulse } from "wacho";
...
```tsx
import { useScope, EffectNode, ERROR } from "wacho";
...
wacho's equivalent of React's `useTransition`, implemented with concurrent lanes. Returns an `[isPending, startTransition]` tuple.
...
```tsx
import { PulseNode, useZoTransition, usePulse } from "wacho";
...
```tsx
import { useLane, usePulse, PulseNode } from "wacho";
