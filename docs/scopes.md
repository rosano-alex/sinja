...
Scopes provide two key capabilities in wacho: **ownership** (automatic cleanup of reactive nodes) and **algebraic effects** (a composable error-handling and control-flow mechanism inspired by languages like Koka and OCaml 5).
...
```ts
import { Scope, PulseNode, ComputedNode, EffectNode } from "wacho";
...
```ts
import { defineEffect } from "wacho";
...
wacho provides three built-in effect keys:
