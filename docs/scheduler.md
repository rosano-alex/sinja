...
wacho uses a deterministic scheduler with priority lanes to control when effects execute. This guarentees that updates happen in a predictable order, regardless of when signals are written.
...
```ts
import { EffectNode, LaneTypes } from "wacho";
...
wacho maintains a global epoch counter (in `clock.ts`) that increments every time any pulse is set. Computed nodes use this epoch to determine if they might be stale — if their `lastEpoch` doesn't match the global epoch, they know a dependency might have changed and they need to recheck.

```ts
import { epoch, tick } from "wacho";
